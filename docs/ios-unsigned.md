...
原生桥接位于 `src-tauri/plugins/lyric-pip`，使用 AVKit 的样本缓冲画中画接口，不引入第二套音频播放器或歌词解析器。只传当前曲目的行文本和播放锚点；窗口播放时每 200 毫秒检查一次歌词，仅内容变化才生成 640×360 图片帧，关闭后释放定时器和图片。前端进度消息合并为一个待发送锚点，避免队列堆积。
