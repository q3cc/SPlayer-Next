...
临时诊断版在每次原生启动时创建 `Library/logs/YYYY-MM-DD_HH-mm-ss.SSS+时区.log`（相对于 iOS 应用数据沙盒）。控制台、未处理异常、原生标准输出/错误和登录请求状态写入同一文件；Cookie、二维码密钥、令牌等凭据脱敏，不记录请求体。历史文件不会自动覆盖或删除。队列过载会记录丢弃数量，操作系统直接杀进程时尚未落盘的最后一批输出可能丢失。
